...
def add_transaction(transaction: dict) -> dict:
    with Session(engine) as session:
        new_t = TransactionModel(sklep=transaction["sklep"], kwota=transaction["kwota"], kategoria=transaction["kategoria"], data=transaction["data"])
        session.add(new_t)
        session.commit()
        session.refresh(new_t)
        response: dict = {"id": new_t.id, "sklep": new_t.sklep, "kwota": new_t.kwota, "kategoria": new_t.kategoria, "data": new_t.data}
